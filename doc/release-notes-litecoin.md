Humanscoin Core version *0.15.1* is now available from:

  <https://download.humanscoin.org/humanscoin-0.15.1/>
...
A number of changes to the way Humanscoin Core deals with peer connections and invalid blocks
...
The minimum relay fee `-minrelayfee` has been lowered to 0.01 humes per kB (0.00001 HUM). This is to prepare for dropping the minimum transaction fee to 0.1 humes per kB (0.0001 HUM) in the next release.
