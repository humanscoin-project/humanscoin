...
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 9633
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 9634
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 19633
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 19634
...
if [ $1 -eq 0 ]; then
	if [ `%{_sbindir}/sestatus |grep -c "disabled"` -eq 0 ]; then
	%{_sbindir}/semanage port -d -p tcp 9633
	%{_sbindir}/semanage port -d -p tcp 9634
	%{_sbindir}/semanage port -d -p tcp 19633
	%{_sbindir}/semanage port -d -p tcp 19634
	for selinuxvariant in %{selinux_variants}; do
